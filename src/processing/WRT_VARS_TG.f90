    SUBROUTINE WRT_INSTANT_VARS_TG
        ...
        DO IVEL=1,NDV+1
            ...
            IF(IVEL.EQ.1) WRITE(WRT_RST_FNM_TG(IVEL),'(A)') TRIM(filepath1)//'DNS_perixz_INSTANT_T'//TRIM(PNTIM)//'_U.D'
            IF(IVEL.EQ.2) WRITE(WRT_RST_FNM_TG(IVEL),'(A)') TRIM(filepath1)//'DNS_perixz_INSTANT_T'//TRIM(PNTIM)//'_V.D'
            IF(IVEL.EQ.3) WRITE(WRT_RST_FNM_TG(IVEL),'(A)') TRIM(filepath1)//'DNS_perixz_INSTANT_T'//TRIM(PNTIM)//'_W.D'
            IF(IVEL.EQ.4) WRITE(WRT_RST_FNM_TG(IVEL),'(A)') TRIM(filepath1)//'DNS_perixz_INSTANT_T'//TRIM(PNTIM)//'_P.D'
            
            !========================RE-ASSIGN VARIABLES=======================================
            DO I=1,NCL1_tg
                DO J=1,N2DO(MYID)
                    DO K=1,NCL3
                        IF(IVEL.EQ.1) DUMMY(I,J,K)=Q_TG(I,J,K,1) !U 
                        IF(IVEL.EQ.2) DUMMY(I,J,K)=Q_TG(I,J,K,2) !V
                        IF(IVEL.EQ.3) DUMMY(I,J,K)=Q_TG(I,J,K,3) !W
                        IF(IVEL.EQ.4) DUMMY(I,J,K)=PR_TG(I,J,K)  !P
                    ENDDO
                ENDDO
            ENDDO
...
!======================================================================================================================
    SUBROUTINE WRT_AVERAGE_VARS_TG
        ...
        IF(N.NE.NSZ) THEN
            WRITE(*,*) 'ERROR in WRT_AVERAGE_VARS_Xperiodic_TG', N, NSZ
        END IF
